function p=autoset(p,pos,graf=0)
  if nargin==1
    pos=[0;0];
  endif
  if p.clase!='auto' 
  ...
  if size(pos)!=[1 1]
    ...
    if p.modelo=='con deposito'
      p.vertices=pos.+[0,0,0.5*(p.long.base-p.long.ventanas),...
      0.5*(p.long.base-p.long.ventanas),p.long.base-0.5*(p.long.base-p.long.ventanas),...
